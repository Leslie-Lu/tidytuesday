# Fri Nov 24 19:05:33 2023 ------------------------------



# .libPaths(c("C:/Library/Applications/r/install/packages"))
# usethis::edit_r_profile()
.libPaths()

# install.packages(c("tidyverse", 'geofacet', 'camcorder'))
library(tidyverse)
library(geofacet)
library(camcorder)
# install.packages('shadowtext')

gg_record(dir = "./tmp/", device = "png", width = 9, height = 8, units = "in", dpi = 800)
...
# or
diwali_sales= house %>% 
  janitor::clean_names() #get unique names
...
ds <- diwali_sales %>% 
  ...
  mutate(
    ...
    product_category = fct_reorder(product_category, abs(med_amount)), #reorder by absolute value
